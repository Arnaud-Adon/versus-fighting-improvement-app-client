...
export default function* userSaga() {
  ...
  yield takeLatest(action.ADD_CHARACTER, userCharacterWorker);
}
...
export function* userCharacterWorker(action) {
  try {
    const user = yield call(ApiService.addUserCharacter, action.payload);
    yield put(fetchUser(user));
    yield rootNavigation.navigate("Improve");
  } catch (error) {
    yield put(fetchFailure(error));
  }
}
